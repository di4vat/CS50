...
char* indexes(const char* path)
{
    ...
    // scanning a directory and checking the entries one by one  
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            // namelist stores the entries of directory
            free(namelist[i]); 
        }
    }
...
{
    // TODO
    BYTE* memory = malloc(sizeof(BYTE));
    ...
    // reading chars one  by one from file
    for (int c = fgetc(file) ; c != EOF ; c = fgetc(file)) 
    {
        memory[size] = c;
        size++;
        
        // increasing size of memory by 1 for the next iteration
        memory = realloc(memory, (size + 1)); 

    }
    
    // adding a null terminator at the end of the contents of the file  
    //memory[size] = '\0';
    // and not strlen(memory) because that would just work for   TEXT files and not IMAGE files
    *length = size; 
    // you can't do content = &memory i.e. u can't change content but u can chnge what it points to
    *content = memory;  
    return true;
}
...
{
    // TODO
    //const char* MIME_type = strrchr(path,".");
    const char* MIME_type = strcasestr(path, ".");
    
    if (MIME_type == NULL) 
        return NULL;
    
    // No need to convert to lowercase  using strcasecmp
    // determine the content type of file through the extension. 

    if ((strcasecmp(".css", MIME_type)) == 0)
    {
        const char* content = "text/css";
        return content;
    }
    else if ((strcasecmp(".html", MIME_type)) == 0)
    {
        const char* content = "text/html";
        return content;
    }
    else if ((strcasecmp(".php", MIME_type)) == 0)
    {
        const char* content = "text/x-php";
        return content;
    }
    else if ((strcasecmp(".js", MIME_type)) == 0)
    {
        const char* content = "text/javascript";
        return content;
    }
    else if ((strcasecmp(".gif", MIME_type)) == 0)
    {
        const char* content = "image/gif";
        return content;
    }
    else if ((strcasecmp(".png", MIME_type)) == 0)
    {
        const char* content = "image/png";
        return content;
    }
    else if ((strcasecmp(".ico", MIME_type)) == 0)
    {
        const char* content = "image/x-icon";
        return content;
    }
    else if ((strcasecmp(".jpg", MIME_type)) == 0)
    {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // from request target
    char* after_get = strchr(line,'/');

    // version not supported
    // one space after GET checked , the second one before HTTP
    ...
    // null terminating the query and abs path  
    abs_path[c] = '\0';  
    query[k] = '\0';
